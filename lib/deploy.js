...
const OSWAP_OtcLiquidityProvider_1 = require("./contracts/restricted/OSWAP_OtcLiquidityProvider");
const OSWAP_OtcPairCreator_1 = require("./contracts/restricted/OSWAP_OtcPairCreator");
const OSWAP_OtcPairOracle_1 = require("./contracts/restricted/OSWAP_OtcPairOracle");
...
async function deployCoreContracts(wallet, options) {
    ...
    result.factory = await factory.deploy({
        ...
        protocolFee: 0,
        protocolFeeTo: options.amm.protocolFeeTo || eth_contract_1.nullAddress,
        tradeFee: 0
    });
...
async function deployOracleContracts(wallet, options, coreContractsResult) {
    ...
    result.oracleFactory = await oracleFactory.deploy({
        ...
        protocolFeeTo: options.protocolFeeTo || eth_contract_1.nullAddress,
...
async function deployRestrictedContracts(wallet, options, weth) {
    ...
    if (!options.pairCreator) {
        ...
        if (options.type == 'Otc') {
            restrictedPairCreator = new OSWAP_OtcPairCreator_1.OSWAP_OtcPairCreator(wallet);
        }
        else {
            restrictedPairCreator = new index_1.OSWAP_RestrictedPairCreator1(wallet);
        }
    ...
    if (options.type == 'Otc') {
        restrictedLiquidityProvider = new OSWAP_OtcLiquidityProvider_1.OSWAP_OtcLiquidityProvider(wallet);
    }
    else {
        restrictedLiquidityProvider = new index_1.OSWAP_RestrictedLiquidityProvider1(wallet);
    }
...
async function deployRestrictedPairOracle(wallet, isOtc) {
    ...
    if (isOtc) {
        restrictedPairOracle = new OSWAP_OtcPairOracle_1.OSWAP_OtcPairOracle(wallet);
    }
    else {
        restrictedPairOracle = new index_1.OSWAP_RestrictedPairOracle(wallet);
    }
